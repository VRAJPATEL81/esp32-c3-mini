...
lv_obj_t *label_heart_rate;
lv_obj_t *label_spo2;

void update_health_data(float heartRate, float spo2) {
    static char heartRateText[16];
    static char spo2Text[16];

    sprintf(heartRateText, "HR: %.1f BPM", heartRate);
    sprintf(spo2Text, "SpO2: %.1f %%", spo2);

    lv_label_set_text(label_heart_rate, heartRateText);
    lv_label_set_text(label_spo2, spo2Text);
}

void create_health_screen() {
    lv_obj_t *screen = lv_scr_act(); // Get current screen

    label_heart_rate = lv_label_create(screen);
    lv_label_set_text(label_heart_rate, "HR: -- BPM");
    lv_obj_align(label_heart_rate, LV_ALIGN_CENTER, 0, -20);

    label_spo2 = lv_label_create(screen);
    lv_label_set_text(label_spo2, "SpO2: -- %");
    lv_obj_align(label_spo2, LV_ALIGN_CENTER, 0, 20);
}
...
void loop() {
    if (max30102_read(&heartRate, &spo2)) {
        update_health_data(heartRate, spo2);
    }
